...
- `version`: release version to download when no binary is found. Default: `v0.2.7`.
...
- `manifest-path`
- `review-output`
- `binary-path`: resolved CLI path. Downstream steps can call this directly, for example to run fully flagged `devimg ai consent`, `devimg review --ai`, `devimg alt`, or `devimg draft` dry-runs after the Action has resolved or downloaded the binary.
