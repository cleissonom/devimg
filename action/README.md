...
- `version`: release version to download when no binary is found. Default: `v0.2.6`.
...
- `manifest-path`
- `review-output`
- `binary-path`: resolved CLI path. Downstream steps can call this directly, for example to run `devimg ai consent --dry-run`, `devimg review --ai --dry-run`, `devimg alt --dry-run`, or `devimg draft --dry-run` after the Action has resolved or downloaded the binary.
