...
- `version`: release version to download when no binary is found. Default: `v0.2.3`.
...
- `manifest-path`
- `review-output`
- `binary-path`: resolved CLI path. Downstream steps can call this directly, for example to run `devimg ai consent --dry-run` after the Action has resolved or downloaded the binary.
