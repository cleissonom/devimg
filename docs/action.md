...
The default `version` input is `v0.2.5`, which maps to release assets such as `devimg-linux-x86_64.tar.gz`. The Action downloads the matching `.sha256` file and verifies the archive before extraction.
...
- `manifest-path`
- `review-output`
- `binary-path`: resolved CLI path. Downstream steps can use it for follow-up commands such as `devimg ai consent --dry-run`, `devimg review --ai --dry-run`, or `devimg alt --dry-run` without re-resolving the binary.
