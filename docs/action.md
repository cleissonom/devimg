...
Set `fail-on-warning: true` only when advisory warnings should block CI. This includes `quality:` diagnostics for low lossy quality, upscaling, risky cover crops, and output-size surprises.
...
The default `version` input is `v0.1.10`, which maps to release assets such as `devimg-linux-x86_64.tar.gz`.
